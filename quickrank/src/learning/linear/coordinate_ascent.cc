...
CoordinateAscent::CoordinateAscent(unsigned int num_points, double window_size,
		...
		num_samples_(num_points), window_size_(window_size), reduction_factor_(
				reduction_factor), max_iterations_(max_iterations), max_failed_vali_(
				max_failed_vali) {
}
...
void CoordinateAscent::learn(
	...
	if (taskID != 0) {
		std::cout << "Connected to MySQL DB, scores will be saved into it too!" << std::endl;
	} else {
		std::cout << "Connection to MySQL DB not established, scores will just be printed to the standard output!" << std::endl;
	}
	...
	for (unsigned int b = 0; b < max_iterations_; b++) {
		...
		for (unsigned int i = 0; i < num_features; i++) {
			...
			preCompute(training_dataset->at(0, 0), n_train_instances,
					num_features, &PreSum[0], &weights[0], &MyTrainingScore[0],
					i);
			...
			for (double lower_bound = weights[i] - window_size;
					lower_bound <= weights[i] + window_size; lower_bound +=
							step) {
				if (lower_bound >= 0)
...
#pragma omp parallel for
			for (unsigned int p = 0; p < points.size(); p++) {
				...
				MyNDCGs[p] = scorer->evaluate_dataset(training_dataset,
						&MyTrainingScore[n_train_instances * p]);
			}
			// End parallel

			// Find the best NDCG
			auto i_max_ndcg = std::max_element(MyNDCGs.cbegin(),
					MyNDCGs.cend());
		...
		std::cout << std::setw(7) << b + 1 << std::setw(9)
				<< metric_on_training;

		// check if there is validation_dataset
		if (validation_dataset) {
			//compute scores of validation documents
			for (unsigned int j = 0; j < validation_dataset->num_instances();
					j++)
				MyValidationScore[j] = std::inner_product(weights.cbegin(),
						weights.cend(), validation_dataset->at(j, 0),
						(Score) 0.0);
			...
			if (taskID != 0) {
				dbconn::insert_training_and_validation_scores(taskID, b + 1,
						metric_on_training, metric_on_validation, bestScore);
				}
		} else {
			//save training results only (if connected to DB)
			if (taskID != 0) {
				dbconn::insert_training_score(taskID, b + 1, metric_on_training);
			}
		}
	...
	if (taskID != 0) {
		dbconn::set_final_training_scores(taskID, window_size, Bestmetric_on_validation, elapsed.count());
	}
...
std::ofstream& CoordinateAscent::save_model_to_file(std::ofstream& os) const {
	...
	for (unsigned int i = 0; i < best_weights_.size(); i++) {
		...
		os
				<< std::setprecision(
						std::numeric_limits<quickrank::Score>::digits10);
