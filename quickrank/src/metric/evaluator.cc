...
void Evaluator::training_phase(std::shared_ptr<learning::LTR_Algorithm> algo,
		...
		const std::string feature_filename, const std::string output_filename,
		const unsigned int npartialsave, const unsigned int taskID) {
	...
	algo->learn(training_dataset, validation_dataset, train_metric,
			npartialsave, output_filename, taskID);
...
void Evaluator::testing_phase(std::shared_ptr<learning::LTR_Algorithm> algo,
		...
		const std::string test_filename, const std::string scores_filename,
	...
	if (test_metric and !test_filename.empty()) {
		...
		std::shared_ptr<quickrank::data::Dataset> test_dataset =
				reader.read_horizontal(test_filename);
		...
		//store test metric to DB (if connected)
		if (taskID != 0) {
			dbconn::set_testing_score(taskID, test_score);
		}
